...
class CustomerFilter(django_filters.FilterSet):
    ...
    def filter_phone_pattern(self, queryset, name, value):
        # Filters phones starting with the given value, e.g. "+1"
        return queryset.filter(phone__startswith=value)

    class Meta:
        model = Customer
        fields = ['name', 'email', 'phone']  # Only real model fields here
...
class ProductFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Product
        fields = ['name', 'price', 'stock']  # Only model fields here
...
class OrderFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Order
        fields = ['total_amount', 'order_date', 'customer']  # Only actual model fields here
